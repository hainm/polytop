...
UI.Panel.prototype.addEntry = function (label, entry) {
  entry.setWidth('210px')
  this
    .add(new UI.Break())
    .add(new UI.Text(label).setWidth('70px'))
...
UI.Selectize = function (idName) {
  ...
  var dom = document.createElement('select')
  dom.className = 'selectize-control'
...
UI.Selectize.prototype.addTo = function (parent, label) {
  ...
  parent.addEntry(label, container)
...
UI.Selectize.prototype.init = function () {
  ...
  self.setOptGroups(self.optgroups)
...
UI.Selectize.prototype.setOptGroups = function (optGroups) {
  var selected = this.dom.value
  var self = this;
  this.clear()

  optGroups.forEach(function (keyVal) {
    self.addOptionGroup(keyVal[0], keyVal[1])
  })

  self.selectize.refreshItems()

  return this
  
}

UI.Selectize.prototype.setValue = function (value) {
...
UI.Selectize.prototype.setOptions = function (options) {
  ...
  self.selectize.addOption(options)
  
  // self.selectize.refreshOptions()
  // self.setValue(value)
  // self.selectize.refreshState()

  return this
}

UI.Selectize.prototype.clear = function () {
